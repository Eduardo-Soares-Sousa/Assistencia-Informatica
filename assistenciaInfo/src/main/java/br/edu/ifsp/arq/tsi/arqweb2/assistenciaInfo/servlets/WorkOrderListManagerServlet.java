...
public class WorkOrderListManagerServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        Employee funcionario = (Employee) request.getSession().getAttribute("funcionarioLogado");

        if(funcionario != null){
            ServiceOrderDao serviceOrderDao = new ServiceOrderDao(DataSourceSearcher.getInstance().getDataSource());
            List<ServiceOrder> serviceOrders = serviceOrderDao.getServiceByFuncionario(funcionario);
